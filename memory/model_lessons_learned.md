...
| 2026-07-09 | Agent | 新增 1.7 超长会话递归思考循环致 UI 冻结条目（飞书 167 轮崩溃） | glm-5.2 在 167 轮长会话中进入递归思考循环；叠加流式输出 + 未重定向的 Popen 输出，前端 UI 超载、输入框禁用 |
...
| v1.4 | 2026-07-09 | 新增 1.7 超长会话递归思考循环致 UI 冻结条目（飞书 167 轮崩溃复盘） | 用户报告上一线程崩溃，要求记录事件并建立常规复盘反馈机制 | Agent |


## 1.7 模型：长会话下的递归思考循环与流式 UI 冻结（2026-07-09 验证）

| 字段 | 内容 |
|------|------|
| **模型标识** | `zhipu-glm-5.2` / 长会话（≥150 轮）、流式输出环境 |
| **适用环境** | Generic Agent 浏览器前端；任务为复杂多步骤工程优化（如飞书 feishu-bot 改造）；持续 1.5 小时以上 |
| **不适用环境** | 短会话（<20 轮）、非流式输出终端、纯命令行工具调用 |
| **现象** | AI 进入同一思考循环，反复输出"让我先.../我需要..."，但无实际进展；流式响应不断追加到 UI 底部，聊天框无法输入，最终用户需刷新页面恢复 |
| **根因** | ① 超长上下文导致模型注意力漂移，对近期指令过度权重，重复同一意图；② 前端流式渲染大量长文本时 UI 线程阻塞；③ 同时 Popen 启动的 `-NoExit` + `Tee-Object` PowerShell 子进程持续向控制台输出，进一步拖慢前端 |
| **验证** | 会话文件 `temp/model_responses/model_responses_622948.txt` 167 轮、1.3MB；截图 `C:/Users/wangx/Desktop/Snipaste_2026-07-09_15-21-44.png`；修复后 `start_feishu_agent.ps1` 已移除 `-NoExit`/`Tee-Object`，改用 `cmd /c` 重定向到文件 |
| **变更后需重新验证** | 超过 100 轮的长会话是否仍出现递归思考；前端是否在流式输出期间保持输入框可用；子进程是否仍有控制台输出泄露 |
