...
## 0. GUI操作节奏要求
进入新界面时，必须先只探测不操作：枚举窗口 + UIA + ljqCtrl截图 + ui_detect，读完实际输出再决定下一步
明确一个操作后，应在同一轮执行该动作，短暂等待，再立刻枚举窗口 + 截图/ui_detect 验证新状态；不要在未知状态下把多步决策写进大脚本
禁止预测关键词筛候选，应看 detect 输出、坐标、层级和上下文判断
